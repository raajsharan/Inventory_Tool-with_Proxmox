...
const STATEMENTS = [
  ...
  // ── Agent Push — WinRM/SSH-based bulk ME Agent deployment, ported from the
  // standalone agent_push_webapp Flask app (its RemCom/pywinpty Windows push
  // paths can't run on this Linux backend; Windows push here goes through
  // WinRM instead — see agentPushWindowsPush.js). Targets are picked from
  // existing Assets/Ext. Assets records (like Test Deploy/Software Status),
  // not typed in — credentials are resolved fresh from the asset record's
  // own asset_password_encrypted at push time, never duplicated here.
