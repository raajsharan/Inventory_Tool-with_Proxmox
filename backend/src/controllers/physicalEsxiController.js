...
const COLUMNS = [
  ...
  { key: 'total_disks_unit',   header: 'Disk Unit (GB/TB)',   width: 16 },
...
async function update(req, res, next) {
  try {
    ...
    teams.notifyAssetUpdate(before, asset, 'physical_esxi_servers', req.user?.full_name || req.user?.email).catch(() => {});
    res.json(asset);
  } catch (e) { next(e); }
}
...
async function downloadTemplate(_req, res, next) {
  try {
    ...
    ws.addRow({
      ...
      cpu_cores: 32, ram_gb: 128, total_disks: 4, total_disks_unit: 'TB', ome_status: 'Active',
