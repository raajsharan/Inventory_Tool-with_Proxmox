...
async function create(req, res, next) {
  try {
    ...
    teams.notifyNewAsset(asset, 'beijing_assets', req.user?.full_name || req.user?.email).catch(() => {});
...
async function update(req, res, next) {
  try {
    ...
    teams.notifyAssetUpdate(before, asset, 'beijing_assets', req.user?.full_name || req.user?.email).catch(() => {});
    res.json(asset);
  } catch (e) { next(e); }
}
