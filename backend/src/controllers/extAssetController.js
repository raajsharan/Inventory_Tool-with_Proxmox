...
async function create(req, res, next) {
  try {
    ...
    teams.notifyNewAsset(asset, 'ext_assets', req.user?.full_name || req.user?.email).catch(() => {});
...
async function update(req, res, next) {
  try {
    ...
    teams.notifyAssetUpdate(before, asset, 'ext_assets', req.user?.full_name || req.user?.email).catch(() => {});
    res.json(asset);
  } catch (e) { next(e); }
}
...
async function viewPassword(req, res, next) {
  try {
    ...
    await audit.log({ user: req.user, action: 'VIEW_PASSWORD', entityType: 'ext_asset', entityId: req.params.id, ipAddress: req.ip });
