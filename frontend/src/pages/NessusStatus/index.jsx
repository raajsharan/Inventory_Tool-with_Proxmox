      ...
      <Row gutter={16} style={{ marginBottom: 20 }}>
        <Col xs={12} sm={6}>
          <StatCard index={0} title="Total Active VMs" value={overall.total}
            icon={<SafetyCertificateOutlined />} color={ACCENT} bg="rgba(22,119,255,0.12)" />
        </Col>
        <Col xs={12} sm={6}>
          <StatCard index={1} title="Nessus Installed" value={overall.installed}
...
function VerifyDetail({ vm, result }) {
  ...
  // SSH connected but the account is locked to a non-interactive shell, so
  // the diagnostic command never ran. Falling through to the normal view
  // would read as "checked, agent missing", which isn't what happened.
