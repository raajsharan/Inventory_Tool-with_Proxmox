...
function statusTag(status) {
  if (status === 'running') return <Tag color="success">Running</Tag>;
  if (status === 'stopped') return <Tag color="default">Stopped</Tag>;
  return <Tag>{status || '—'}</Tag>;
}

export default function PVEMacLookup() {
  ...
  const load = useCallback((q = search, mf = matchFilter, sf = statusFilter) => {
    setLoading(true);
    const params = {};
    if (q)  params.search      = q;
    if (mf) params.matchFilter = mf;
    if (sf) params.status      = sf;
    api.get('/proxmox/mac-lookup', { params })
      .then(r => { setData(r.data); setError(null); })
      .catch(e => setError(e?.response?.data?.error || e.message || 'Failed to load MAC lookup data.'))
      .finally(() => setLoading(false));
  }, [search, matchFilter, statusFilter]); // eslint-disable-line

  useEffect(() => { load(); }, []); // eslint-disable-line

  function onSearch() { load(search, matchFilter, statusFilter); }

  async function onExport() {
    const token = localStorage.getItem('token');
    const resp  = await fetch('/api/proxmox/mac-lookup/export', {
      headers: { Authorization: `Bearer ${token}` },
    });
    const blob = await resp.blob();
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'proxmox_mac_lookup.csv';
    a.click();
  }
  ...
  const columns = [
    ...
    { title: 'Node', dataIndex: 'node', key: 'node', ellipsis: true, width: 140 },
          ...
          extra={
            <Space wrap>
              ...
              <Select
                ...
                onChange={v => { setMatchFilter(v || ''); load(search, v || '', statusFilter); }}
                style={{ width: 140 }}
              >
              ...
              <Select
                ...
                onChange={v => { setStatusFilter(v || ''); load(search, matchFilter, v || ''); }}
                style={{ width: 130 }}
              >
