...
import {
  Alert, Badge, Button, Card, Col, Descriptions, Form, Input,
...
export default function SoftwareStatus() {
  ...
  // Open the Endpoint dialog pre-filled from the asset record
  ...
  const runVerify = useCallback(async (vm, overrides = {}) => {
    ...
    try {
      const { data: r } = await api.post('/software-status/verify', {
  ...
  const runInstall = useCallback(async (vm, overrides = {}) => {
    ...
    // For Linux: always show the Endpoint dialog to confirm / collect credentials first,
    // unless this call is already coming from the dialog (overrides.fromDialog).
    ...
    try {
      const { data: r } = await api.post('/software-status/install', {
        ip_address: vm.ip_address, source: vm.source,
        port: overrides.port || 22,
        override_username: overrides.username || undefined,
        override_password: overrides.password || undefined,
  ...
  // credential/endpoint modal submit (shared)
    ...
    return data.locations
      ...
      .map(r => {
        const vms = (r.vms || []).filter(v => {
          if (filterStatus === 'installed'     && !v.me_installed) return false;
          if (filterStatus === 'not_installed' && v.me_installed)  return false;
  ...
  const vmColumns = [
    ...
    },
    {
      title: (
        <Space>
          Live Check
          <Tooltip title="SSH: checks service status + binary file">
    ...
    ...(isAdmin ? [{
      title: (
        <Space>
          Install
          <Tooltip title="Select method then click Install. 'Try All' auto-tries every method until one succeeds.">
            <InfoCircleOutlined style={{ color: '#8c8c8c' }} />
          </Tooltip>
        </Space>
      ),
  ...
  const locColumns = [
    ...
    { title: 'Not Installed',dataIndex: 'not_installed', width: 120, align: 'right', render: v => <span style={{ color: v > 0 ? '#ff4d4f' : '#8c8c8c' }}>{v}</span> },
    {
      ...
      render: (pct, r) => (
        <Space>
          <Progress percent={pct} size="small" strokeColor={complianceColor(pct)}
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <Space align="start">
          ...
          <div>
            <Typography.Title level={4} style={{ margin: 0 }}>Software Status</Typography.Title>
            <Typography.Text type="secondary">
              ManageEngine Agent — verify &amp; install via SSH across Windows and Linux VMs
            </Typography.Text>
          </div>
        </Space>
        {isAdmin && (
      ...
      <Modal
        ...
        footer={[
          <Button key="save" type="primary" onClick={() => credForm.submit()}>
  ...
  return (
    <Space direction="vertical" style={{ width: '100%' }} size={16}>
      ...
      <Alert type={result.installed ? 'success' : 'warning'} showIcon
        message={result.installed ? 'ManageEngine Agent is installed.' : 'ManageEngine Agent is not installed.'} />
      {svc.output && (
        <>
...
function InstallDetail({ vm, result }) {
  ...
  return (
    <Space direction="vertical" style={{ width: '100%' }} size={16}>
      ...
      <Alert
        type={success ? 'success' : 'warning'} showIcon
        message={success
          ? 'Install completed successfully. Click "Verify Now" to confirm the agent is running.'
